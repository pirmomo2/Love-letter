			...
			for (let t = 1; t < 7; t++) {
				...
				((d[t].velocity.x += 0.01 * c.X),
					...
					d[t].position.y >= o - 11 - 1 &&
						(d[t].velocity.y > 0 &&
							(d[t].velocity.y = 0.7 * -d[t].velocity.y),
						(d[t].position.y = o - 11 - 1)),
					d[t].position.x >= s - 11 &&
						(d[t].velocity.x > 0 &&
							(d[t].velocity.x = 0.7 * -d[t].velocity.x),
		...
		((this.rotationSign = Math.random() < 0.5 ? -1 : 1),
			...
			(this.update = function (t) {
				((this.position.x += this.velocity.x),
					...
					this.lifeSpan--,
					this.age++,
					(this.velocity.x += f.x_func(
						this.age,
						this.initialLifeSpan,
					)),
					(this.velocity.y += f.y_func(
						this.age,
						this.initialLifeSpan,
					)));
				const e = p(this.age, this.initialLifeSpan),
					n =
						0.0174533 *
						(this.rotationSign * y(this.age, this.initialLifeSpan));
				(t.translate(this.position.x, this.position.y),
					t.rotate(n),
					t.drawImage(
						this.canv,
